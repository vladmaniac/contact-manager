...
const FIELDS_ERROR = "Name, e-mail and birthday fields must not be empty";
...
Validation.prototype = {
  ...
  checkFields: function Validation_checkFields(aName, aEmail, aBirthday) {
    if(aName === '' || aBirthday === '' || aEmail === '') {
      return false;
    }
  ...
  // Validate fields
  checkAllData: function Validation_checkAllData(errors, aName, aEmail, aBirthday, aAddress) {
    if (this.checkMail(aEmail) && this.checkStrings(aName, aAddress) && this.checkFields(aName, aEmail, aBirthday))
      return true;
    else {
      if (!this.checkMail(aEmail)) 
        errors.push(EMAIL_ERROR);
      if (!this.checkFields(aName, aEmail, aBirthday)) 
