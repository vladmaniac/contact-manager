...
Operations.prototype = {
  ...
  sendToForm: function Operations_sendToForm() {
    ...
    this.getElement("birth-field").value = contact.birthday;
  ...
  edit: function Operations_edit() {
    ...
    newBirthday = this.getElement("birth-field").value;
    newAddress = this.getElement("address-field").value;

    manager.editContact(this.contact, newName, newEmail, newBirthday, newAddress);
    console.log("in edit");
    console.log(manager.contacts);
    handler.updateStorage();
    this.updateUi(newName, newEmail, newBirthday, newAddress);
  },

  // Update the UI after editing a contact
  updateUi: function Operations_updateUi(aName, aMail, aBirthday, aAddress) {
    var name,
      email,
      birth,
      address;
    ...
    birth = document.getElementById("birth-field" + this.contact.id);
    address = document.getElementById("address-field" + this.contact.id);

    name.textContent = "Name: " + aName;
    birth.textContent = "Date of birth: " + aBirthday;
